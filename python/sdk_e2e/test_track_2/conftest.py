...
@pytest.fixture(scope="module")
def paired_client(
    connected_mqtt_client, operation_ticket_list, c2d_subscription, run_id, requested_service_pool
):
    ...
    while time.time() - start_time <= PAIRING_REQUEST_TIMEOUT_INTERVAL_IN_SECONDS:
        ...
        response = operation_ticket.event.wait(timeout=PAIRING_REQUEST_SEND_INTERVAL_IN_SECONDS)
        if response:
            logger.info("pairing response received")
            msg = json.loads(operation_ticket.result_message.payload.decode())
...
@pytest.fixture(scope="module")
def message_wrapper(operation_ticket_list, run_id, paired_client, c2d_subscription):
    def wrapper_function(payload, cmd=None):
        operation_ticket = operation_ticket_list.make_event_based_operation_ticket()

        if Fields.THIEF not in payload:
            payload[Fields.THIEF] = {}
        payload[Fields.THIEF][Fields.OPERATION_ID] = operation_ticket.id
        ...
        return collections.namedtuple("WrappedMessage", "message operation_ticket")(
            message, operation_ticket
        )
