...
def add_logging_properties(
    ...
    session_id=_default_value,
    pool_id=_default_value,
    transport=_default_value,
):
    global _client_type, _run_id, _hub, _sdk_version, _device_id, _session_id, _pool_id, _transport
...
def telemetry_processor_callback(envelope):
    ...
    if _session_id:
        envelope.data.baseData.properties["sessionId"] = _session_id
    if _pool_id:
