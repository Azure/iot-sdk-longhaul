...
class DeviceApp(object):
    ...
    def __init__(self):
        ...
        self.op_ticket_list = RunningOperationList()
        # for metrics
    ...
    def pair_with_service(self):
        ...
        while (time.time() - pairing_start_time) < self.config[
            ...
            op_ticket = self.op_ticket_list.make_event_based_operation()
            pairing_payload = {
                Fields.THIEF: {
    ...
    def test_send_message_thread(self):
        ...
        def send_message():
            ...
            self.send_message_with_metrics(msg, op_ticket.id)

            if op_ticket.event.wait(timeout=self.config[Settings.OPERATION_TIMEOUT_IN_SECONDS]):
                logger.info("Telemetry op {} arrived at service".format(op_ticket.id))
                self.metrics.send_message_count_verified.increment()

            else:
                logger.warning("Telemetry op {} did not arrive at service".format(op_ticket.id))
    ...
    def test_flood_thread(self):
        ...
        def do_flood():
            ...
            all_ops = [
                self.op_ticket_list.make_event_based_operation()
                for x in range(
    ...
    def handle_operation_response(self, msg):
        ...
        for operation_id in operation_ids:
            op_ticket = self.op_ticket_list.get(operation_id)
            if op_ticket:
    ...
    def test_reported_properties(self):
        ...
        add_operation = self.op_ticket_list.make_event_based_operation()
        remove_operation = self.op_ticket_list.make_event_based_operation()
    ...
    def test_method_invoke(self, method):
        ...
        logger.info("sending method invoke with guid={}".format(op_ticket.id))
        self.client.send_message(msg)

        if op_ticket.event.wait(timeout=self.config[Settings.OPERATION_TIMEOUT_IN_SECONDS]):
            logger.info(
                "method with guid {} returned {}".format(op_ticket.id, op_ticket.result_message)
            )
            thief = json.loads(op_ticket.result_message.data.decode())[Fields.THIEF]
            fail = False
            ...
            if thief[Fields.METHOD_RESPONSE_PAYLOAD] != payload:
                logger.error(
                    "Unexpected payload: id={}, received {} expected {}".format(
                        op_ticket.id, thief[Fields.METHOD_RESPONSE_PAYLOAD], payload
                    )
                )
                fail = True
            ...
            logger.error("method with guid {} never completed".format(op_ticket.id))
    ...
    def test_c2d(self):
        ...
        logger.info("Requesting C2D for operationId {}".format(op_ticket.id))
        self.client.send_message(self.create_message_from_dict(command_payload))

        if op_ticket.event.wait(timeout=self.config[Settings.OPERATION_TIMEOUT_IN_SECONDS]):
            logger.info("C2D received for operationId {}".format(op_ticket.id))
            ...
            if thief[Fields.TEST_C2D_PAYLOAD] != sent_test_payload:
                logger.warning(
                    "C2D payload for operationId {}  does not match.  Expected={}, Received={}".format(
                        op_ticket.id, sent_test_payload, thief[Fields.TEST_C2D_PAYLOAD]
                    )
                )
                raise ThiefFatalException("C2D payload mismatch")
        else:
            logger.info("C2D timed out for operationId {}".format(op_ticket.id))
