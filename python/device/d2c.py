...
class App(object):
    ...
    def send_telemetry_thread(self):
        ...
        while not done:
            ...
            app_insights.d2c_metrics.d2c_roundtrip_latency = (
                self.metrics.d2c.latency.extract_average()
            )
            app_insights.d2c_metrics.d2c_in_flight_count = self.metrics.d2c.inflight.get_count()
            app_insights.d2c_metrics.d2c_succeeded_count = (
                self.metrics.d2c.succeeded.extract_count()
            )
            app_insights.d2c_metrics.d2c_failed_count = self.metrics.d2c.failed.extract_count()
            app_insights.d2c_metrics.d2c_waiting_to_verify_count = len(self.d2c_unconfirmed)
            app_insights.d2c_metrics.d2c_verified_count = self.metrics.d2c.verified.extract_count()

            app_insights.d2c_record()

            logger.debug("setting telementry: {}".format(app_insights.d2c_metrics.to_dict()))
